...
function showStep(n) {
  ...
  btnNext.textContent = currentStep === 5 ? 'Close' : 'Next';
}
if (stepRoot) {
  showStep(1);
...
function buildA4Template(title, contentHtml) {
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8">
  ...
  const profileHtml = f.profile ? (function(){
    ...
    return `
      <div class="section" style="text-align:center;">
        ${f.profile.name ? `<h1 style="font-size:14px;margin:0 0 6px 0;font-weight:700;">${escapeHtml(f.profile.name)}</h1>` : ''}
        ${f.profile.position ? `<div style="margin:0 0 8px 0;font-size:13px;font-weight:600;">${escapeHtml(f.profile.position)}</div>` : ''}
        ${metaLine}
        ${hrLine}
        ${summaryBlock}
...
if (generateLetterBtn) generateLetterBtn.addEventListener('click', () => {
  ...
  if (f.objet) {
    ...
    // make the objet area compact (5vh) and indented 60px, and increase the internal top padding for clearer separation
    step4Fragments.push(`<div style="height:42px;"></div>`);
    step4Fragments.push(`<div style="margin-left:60px; min-height:5vh; display:flex; align-items:flex-start; padding-top:18px;">${escapeHtml(f.objet)}</div>`);
  ...
  closePopup(popupStart);
  openPrintWindow(buildA4Template('Letter', content));
});
